...
# MUST load .env BEFORE importing supabase_client
...
async def startup_event():
    admin_exists = await db.find_admin("admin")
    if not admin_exists:
...
async def login(req: LoginRequest):
    if req.role == "admin":
        admin = await db.find_admin(req.identifier)
        ...
        judge = await db.find_judge(req.identifier)
        ...
        team = await db.find_team(req.identifier)
        if not team:
            await db.create_team(req.identifier)
...
async def create_judge(judge: JudgeCreate, payload: dict = Depends(verify_token)):
    ...
    existing = await db.find_judge(judge.judge_id)
    if existing:
...
async def set_team_password(data: TeamPasswordSet, payload: dict = Depends(verify_token)):
    ...
    await db.set_team_config(hashed)
...
async def set_timer(timer: TimerConfig, payload: dict = Depends(verify_token)):
    ...
    await db.set_timer_config(timer.end_time, timer.is_active)
...
async def get_timer_admin(payload: dict = Depends(verify_token)):
    ...
    timer = await db.get_timer_config()
    if not timer:
...
async def get_admin_leaderboard(payload: dict = Depends(verify_token)):
    ...
    for team in teams:
        scores = await db.get_team_scores(team["team_name"])
        if scores:
...
async def submit_score(score_data: ScoreSubmit, payload: dict = Depends(verify_token)):
    ...
    existing = await db.find_score(judge_id, score_data.team_name)
    
    timestamp = datetime.now(timezone.utc).isoformat()
    
    if existing:
        await db.update_score(judge_id, score_data.team_name, score_data.scores, timestamp)
    else:
        await db.create_score(judge_id, score_data.team_name, score_data.scores, timestamp)
...
async def get_team_profile(payload: dict = Depends(verify_token)):
    ...
    team = await db.find_team(team_name)
    if not team:
...
async def update_team_profile(profile: TeamProfile, payload: dict = Depends(verify_token)):
    ...
    await db.update_team(team_name, profile_data)
...
async def get_team_score(payload: dict = Depends(verify_token)):
    ...
    team_name = payload.get("identifier")
    scores = await db.get_team_scores(team_name)
...
async def upload_photo(file: UploadFile = File(...), payload: dict = Depends(verify_token)):
    ...
    await db.update_team(team_name, {"photo_url": photo_url})
